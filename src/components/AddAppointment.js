...
const AddAppointment = ({ onSendAppointment, lastId }) => {
    // Object to clear the form data after submission but also to initialize the form data state
    const clearData = {
        petName: "",
        ownerName: "",
        aptDate: "",
    ...
    // useState is a hook that allows us to have state variables in functional components by importing it from 
    // react and calling it inside the component function body. It is used to create state variables that can hold 
    // and manage data that may change over time.
    // 1. In the toggle feature's case, toggleForm is a boolean state variable that indicates whether the form is visible or not.
    // 2. In the formData's case, formData is an object state variable that holds the values of the form fields.
    let [toggleForm, setToggleForm] = useState(false); // State to show/hide the form
    let [formData, setFormData] = useState(clearData) // State to hold form data
